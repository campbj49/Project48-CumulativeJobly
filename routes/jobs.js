//Modified from provided companies.js
"use strict";
...
router.get("/", async function (req, res, next) {
  try {
    ...
    if(req.query.title) {
        filterString += `LOWER(title) LIKE '%${req.query.title.toLowerCase()}%'`;
        nextAnd = " AND ";
    }
    ...
    if(req.query.hasEquity) 
      filterString += `${nextAnd}equity IS NOT NULL`;

    if(filterString === "WHERE ") filterString = "";

    const jobs = await Job.filter(filterString);
